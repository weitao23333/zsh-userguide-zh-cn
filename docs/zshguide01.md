...
shell can do and how, and that\'s what this guide is for.
...
accident, and that sort of thing. In something like \``rm file`\', I
will often refer to the \`command\' (`rm`, of course) and the
...
administrator should read [Appendix A](zshguide08.html#appa). For now,
...
\``print hello`\', then backspace over \``hello`\' and type
\``goodbye`\'. Now hit the \`Return\' key (or \`Enter\' key, I\'ll just
say `<RET>` from now on, likewise `<TAB>` for the tab key, `<SPC>` for
the space key); unless you have a serious practical-joker problem on
your system, you will see \``goodbye`\', and the shell will come back
with another prompt. All of the time up to when you hit `<RET>`, you
...
the `print` command to print out a message. So you can see that the
shell is important.

However, if all you\'re doing is typing simple commands like that, why
...
-   use zle to minimise the amount of typing you have to do \-\-- in
    zsh, you can even edit small files that way,
...
That\'s only a tiny sample. Since there\'s so much to say, this guide
...
exhaustive, for which you should look at the shell manual.
...
putting an \``&`\' after the command, which means that the shell
doesn\'t wait for them to finish. The shell is there even if the
command\'s in the foreground, but in this case doing nothing.

Second, it doesn\'t just run other people\'s commands, it has some of
its own, called **builtin commands** or just **builtins**, and you can
...
lines with the \``%`\' in front into a terminal to be executed, there\'s
...
parameter itself, rather than its value, you omit the \``$`\'. When you
do and don\'t need it should become clearer as we go along.
...
\``shwordsplit`\', \``nonomatch`\', \``zle`\'. If you\'re philosophical
...
to be ridiculous. I mean, who wrote this? Never mind.
